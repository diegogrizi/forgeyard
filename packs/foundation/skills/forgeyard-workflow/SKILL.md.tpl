...
8. **Close:** in the main checkout, use `forgeyard task complete` with the current receipt. For isolated work, use `forgeyard workspace validate` and then the serialized `forgeyard workspace integrate`; integration stops on conflicts or failed combined-tree verification and never pushes.
